...
public class PlayerInventory : MonoBehaviour {
    ...
    public void AddItem(int type)
    {
        if (type == 0)
        {
            ...
            count += 5;
            platformText[0].text = count.ToString();
            platformsCount[0] = count;
        }
        else
        {
            for (int x = 0; x < platformsCount.Length; x++)
            {
                platformsCount[x] += 3;
                int count = 0;
                string countText = platformText[x].text;
                Int32.TryParse(countText, out count);
                count += 3;
                platformText[x].text = count.ToString();
                platformsCount[x] = count;
            }
        }
    }
